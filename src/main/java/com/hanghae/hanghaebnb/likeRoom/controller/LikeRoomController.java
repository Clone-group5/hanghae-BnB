...
public class LikeRoomController {
    ...
    public ResponseEntity<ResponseDto> likeRoom(@PathVariable Long roomId, @AuthenticationPrincipal UserDetailsImpl userDetails){
        ResponseLikeRoom responseLikeRoom = likeRoomService.likeRoom(roomId, userDetails.getUsers());
        if(responseLikeRoom.getLike()){
            ResponseDto responseDto = new ResponseDto<>(200, "숙소 좋아요가 완료되었습니다.", responseLikeRoom);
            return new ResponseEntity<>(responseDto, HttpStatus.OK);
        }
        else{
            ResponseDto responseDto = new ResponseDto<>(200, "숙소 좋아요가 취소되었습니다.", responseLikeRoom);
            return new ResponseEntity<>(responseDto, HttpStatus.OK);
        }
    }
